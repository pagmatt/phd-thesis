...
The rest of the section is organized as follows. Section~\ref{sec:system} introduces the system model. Section~\ref{sub:THzLinkSel} describes the algorithm for frequency and path selection, which is then numerically evaluated in Section~\ref{sec:PerfEval}.
...
\label{sec:PerfEval}
This section introduces a performance evaluation based on a novel simulation setup (Section~\ref{sec:SimSetup}) in a dense cellular network (Section~\ref{sub:SimScenario}), with a comparison between different results of THz and mmWave networks (Section~\ref{sub:results-iab-thz}).
